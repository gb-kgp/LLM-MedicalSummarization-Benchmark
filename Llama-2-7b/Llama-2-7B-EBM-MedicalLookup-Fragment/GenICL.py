...
def prepend_icl_examples(element,element_idx, prefix1,prefix2,suffix):
    ...
    df = pd.read_json('./bioasq_train.json',orient='records',lines=True)
    trn_inputs = df['inputs'].tolist()
    trn_target = df['target'].tolist()
    
    example_indices = np.load('./closest_neighbors_BioASQ.npy')
...
df_test = pd.read_json('./bioasq_test.json',orient='records',lines=True)
srcs = df_test['inputs'].tolist()
tgts = df_test['target'].tolist()

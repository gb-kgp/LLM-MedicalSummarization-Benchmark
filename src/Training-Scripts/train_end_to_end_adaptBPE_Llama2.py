...
import math
import numpy as np
hf_token = "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"
...
def instantiate_model_by_mean(
    ...
    target_tokenizer: LlamaTokenizer,
    tie_word_embeddings: bool = False
) -> tuple[AutoModelForCausalLM, LlamaTokenizer]:
...
def main(args, training_args):
    ...
    # Load the tokenizer
    tokenizer = LlamaTokenizer.from_pretrained(args.tokenizer_name_or_path, added_vocab = f'{args.tokenizer_name_or_path}/added_vocab.txt',use_fast=False)
    source_tokenizer = AutoTokenizer.from_pretrained(model_id,token=hf_token, use_fast=False)

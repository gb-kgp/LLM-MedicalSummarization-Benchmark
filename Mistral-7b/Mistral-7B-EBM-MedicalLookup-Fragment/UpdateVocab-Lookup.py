...
for fname in glob.glob(f'{args.vocab_path}/*'):
    llama_tokenizer = LlamaTokenizer.from_pretrained('mistralai/Mistral-7B-v0.1')
    llama_spm = sp_pb2_model.ModelProto()
    llama_spm.ParseFromString(llama_tokenizer.sp_model.serialized_model_proto())

    pre_vocab = llama_tokenizer.get_vocab()
    ...
    for idx,row in enumerate(special_tokens):
        ...
        if token in pre_vocab: continue
        new_p = sp_pb2_model.ModelProto().SentencePiece()
        new_p.piece = token
        new_p.score = -1*len(llama_spm.pieces)
        # print(new_p.score)
...
for fname in sorted(glob.glob(f'{args.dir_dump}/*') ,key = lambda x:int(x.split('/')[-1].split('_')[-1])):

...
tok = LlamaTokenizer.from_pretrained('mistralai/Mistral-7B-v0.1')
...
PAC_topK = [x.split()[0] for x in list_PM_All[:args.v_size]]

list_TGT_notPLM = list()
with open(f'VocabFiles_Mistral/{args.dataset}_All.vocab','r') as f:
    for idx,line in enumerate(f):
        if idx < 3: continue
        term = line.split()[0]
        if term in org_vocab: continue
